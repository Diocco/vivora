...
int agregar_usuario(char usuario[H]){
    ...
    if(strcmp(usuario_datos.nombre,"Admin")==0){usuario_datos.banco=100000;} //Si el nombre ingresado es "Admin" se le pone 100000 en el banco
...
int tienda(){
        ...
        for(i=0;i<6;i++){recurrentes.mod_cuadricula(x+i-7,y-10,text_banco[i]);}
        ...
        for(i=0;i<6;i++){recurrentes.mod_cuadricula(x+i+2,y-10,text_valor_banco[i]);}
        /////////////////

        recurrentes.definircuadro2(6,2,-5+5*abajo,-13+13*derecha); //Cuadro de seleccion
        for(i=0;i<(int)strlen(text_volver);i++){recurrentes.mod_cuadricula(x+i-16+13,y+5,text_volver[i]);}
        ...
        for(i=0;i<7;i++){recurrentes.mod_cuadricula(x+i-16+13,y-6,text_piedras[i]);}
        for(i=0;i<5;i++){recurrentes.mod_cuadricula(x+i-16+13,y-5,text_nivel[i]);}
        recurrentes.mod_cuadricula(x+i-15+13,y-5,char(49+n_max_piedras));
        recurrentes.mod_cuadricula(x-16+13,y-4,char(36));
        for(i=0;i<6;i++){recurrentes.mod_cuadricula(x+i-15+13,y-4,temp_precio_text[i]);}
        ////////////////////
        ...
        for(i=0;i<6;i++){recurrentes.mod_cuadricula(x+i-16+26,y-6,text_alimento[i]);}
        for(i=0;i<5;i++){recurrentes.mod_cuadricula(x+i-16+26,y-5,text_nivel[i]);}
        recurrentes.mod_cuadricula(x+i-15+26,y-5,char(49+n_max_alimento));
        recurrentes.mod_cuadricula(x-16+26,y-4,char(36));
        for(i=0;i<6;i++){recurrentes.mod_cuadricula(x+i-15+26,y-4,temp_precio_text[i]);}
        ////////////////////
        ...
        for(i=0;i<(int)strlen(text_multi_adiccional);i++){recurrentes.mod_cuadricula(x+i-18+13,y-1,text_multi_adiccional[i]);}
        for(i=0;i<5;i++){recurrentes.mod_cuadricula(x+i-16+13,y,text_nivel[i]);}
        recurrentes.mod_cuadricula(x+i-15+13,y,char(49+multiplicador_adiccinal));
        recurrentes.mod_cuadricula(x-16+13,y+1,char(36));
        for(i=0;i<6;i++){recurrentes.mod_cuadricula(x+i-15+13,y+1,temp_precio_text[i]);}
        /////////////////////
    ...
    cout <<config.dev_config(1)<< endl;
        ...
        if(derecha==0&&abajo==0){ //Se selecciona aumentar la valocidad
                if(config.dev_banco()<precio_velocidad){recurrentes.error(1);}
                else if(n_max_velocidad>7){recurrentes.error(2);}
                else{
        ...
        if(derecha==1&&abajo==0){ //Se selecciona aumentar el nivel de piedras
                if(config.dev_banco()<precio_piedras){recurrentes.error(1);}
                else if(n_max_piedras>7){recurrentes.error(2);}
                else{
        ...
        if(derecha==2&&abajo==0){ //Se selecciona aumentar el nivel de alimento
                if(config.dev_banco()<precio_alimento){recurrentes.error(1);}
                else if(n_max_alimento>7){recurrentes.error(2);}
                else{
        ...
        if(derecha==1&&abajo==1){ //Se selecciona aumentar el nivel del multiplicador adiccinal
                if(config.dev_banco()<precio_multiplicador){recurrentes.error(1);}
                else if(multiplicador_adiccinal>7){recurrentes.error(2);}
                else{
...
int opciones(){
    ...
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)-6),char(179));}
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)-7),char(194));}
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)-5),char(193));}
    for(k=2;k<npiedras*2+2;k=k+2){recurrentes.mod_cuadricula(R/2+k-8,((R/4)-6),char(219));}
    recurrentes.mod_cuadricula(R/2-8,((R/4)-6),char(219));
    recurrentes.definircuadro2(9,1,0,0);
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)),char(179));}
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)-1),char(194));}
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)+1),char(193));}
    for(k=2;k<nvelocidad*2+2;k=k+2){recurrentes.mod_cuadricula(R/2+k-8,((R/4)),char(219));}
    recurrentes.mod_cuadricula(R/2-8,((R/4)),char(219));
    recurrentes.definircuadro2(9,1,6,0);
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)+6),char(179));}
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)+5),char(194));}
    for(k=0;k<16;k=k+2){recurrentes.mod_cuadricula(R/2+k-7,((R/4)+7),char(193));}
    for(k=2;k<nalimento*2+2;k=k+2){recurrentes.mod_cuadricula(R/2+k-8,((R/4)+6),char(219));}
    recurrentes.mod_cuadricula(R/2-8,((R/4)+6),char(219));
    /////////////////

    // Se definen los titulos de las barras de carga
        for(k=0;k<(int)strlen(piedras);k++){recurrentes.mod_cuadricula(R/2+k-3,((R/4)-8),piedras[k]);}
        for(k=0;k<(int)strlen(velocidad);k++){recurrentes.mod_cuadricula(R/2+k-4,((R/4)-2),velocidad[k]);}
        for(k=0;k<(int)strlen(vacio);k++){recurrentes.mod_cuadricula(R/2+k-4,((R/4)+4),vacio[k]);}
    ////////////
    ...
    for(k=0;k<(int)strlen(multi);k++){recurrentes.mod_cuadricula(R/2+k-8,((R/4)+9),multi[k]);}
...
int tabla(){
        ...
        // Indice
        for(k=0;k<9;k++){recurrentes.mod_cuadricula(R/2-D+1,R/4-A+3+k,char(49+k));}
        ...
        for(k=0;k<(int)strlen(Nombre);k++) recurrentes.mod_cuadricula(R/2-D+k+5,R/4-A+3+i,Nombre[k]);
        }
...
int bienvenido_V2(){
    ...
    for(i=0;i<(int)strlen(text_bienvenido);i++){recurrentes.mod_cuadricula(x+i-5,y-4,text_bienvenido[i]);}
    for(i=0;i<(int)strlen(text_ingrese);i++){recurrentes.mod_cuadricula(x+i-14,y-2,text_ingrese[i]);}
    recurrentes.dibujar();
    ///////////////////////////////
    ...
    do{
        ...
        if (!((int)nombre[k]==8&&k==0)){for(i=0;i<(int)strlen(nombre);i++){recurrentes.mod_cuadricula(x+i-1-int(k/2),y,nombre[i]);} //Se escribe la tecla ingresada a menos que se haya apretado el retroceso y no habia nada escrito.
                                         k++;}
    ...
    cout << "Se busca" <<nombre << endl ; Sleep(1500);
        if(config.buscar_usuario(nombre)!=-1)//Si se encuentra el nombre ingresado:
            ...
            for(i=0;i<(int)strlen(text_encontrado);i++){recurrentes.mod_cuadricula(x+i-9,y+2,text_encontrado[i]);} //Se escribe el texto en pantalla que se encontro el usuario
            e=2;
            recurrentes.dibujar();
            Sleep(2000);
        ...
        {
            for(i=0;i<(int)strlen(text_noencontrado);i++){recurrentes.mod_cuadricula(x+i-10,y+2,text_noencontrado[i]);} //Se escribe el texto en pantalla que no se encontro el usuario
            recurrentes.dibujar();

            do{// Se le da eleccion al usuario si elegir entre volver a escribir el usuario o de agregarlo a la base de datos
            recurrentes.definircuadro(19,6);

            if(e==0){recurrentes.definircuadro2(8,1,4,-9);} //Seleccion sobre Agregar Usuario
            if(e==1){recurrentes.definircuadro2(9,1,4,9);} //Seleccion sobre Volver a escribir

            //Se vuelven a definir todos los objetos en pantalla
            for(i=0;i<(int)strlen(text_volver);i++){recurrentes.mod_cuadricula(x+i+1,y+4,text_volver[i]);}
            for(i=0;i<(int)strlen(text_agregar);i++){recurrentes.mod_cuadricula(x+i-16,y+4,text_agregar[i]);}
            for(i=0;i<(int)strlen(text_bienvenido);i++){recurrentes.mod_cuadricula(x+i-5,y-4,text_bienvenido[i]);}
            for(i=0;i<(int)strlen(text_ingrese);i++){recurrentes.mod_cuadricula(x+i-14,y-2,text_ingrese[i]);}
            for(i=0;i<(int)strlen(text_noencontrado);i++){recurrentes.mod_cuadricula(x+i-10,y+2,text_noencontrado[i]);}
            k=strlen(nombre);
            for(i=0;i<k;i++){recurrentes.mod_cuadricula(x+i-1-int(k/2),y,nombre[i]);}
            ////////////////////////////
    ...
    if(e==0){ //Si se elige "Agregar usuario" se va agregar el usuario en el archivo de usuarios existentes
            ...
            recurrentes.definirmatriz();
            for(i=0;i<(int)strlen(text_agregado);i++){recurrentes.mod_cuadricula(x+i-8,y+3,text_agregado[i]);}
            for(i=0;i<(int)strlen(text_ingrese);i++){recurrentes.mod_cuadricula(x+i-14,y-2,text_ingrese[i]);}
            for(i=0;i<(int)strlen(text_bienvenido);i++){recurrentes.mod_cuadricula(x+i-5,y-4,text_bienvenido[i]);}
            k=strlen(nombre);
            for(i=0;i<k;i++){recurrentes.mod_cuadricula(x+i-1-int(k/2),y,nombre[i]);}
...
int main()
{
    ...
    if(IU.menu()==4){config.guardar_config();
                     return 0;}
